...
describe('iOS SDK sources', () => {
  ...
  it('is a Swift package that uses URLSession and exposes the intended API', () => {
    ...
    expect(combined).toContain('static func start')
    expect(combined).toContain('DeliveryAttributes')
    expect(combined).toContain('DeliveryLiveActivity')
  ...
  it('retries transient failures and observes token rotation in tests', () => {
    ...
    expect(tests).toContain('testLiveActivitiesPlistHelper')
    expect(tests).toContain('testSDKVersionIsInUserAgent')
  })
})
