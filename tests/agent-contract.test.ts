...
describe('agent contract', () => {
  it('llms.txt states the golden path, keys, and do-not list', () => {
    ...
    expect(LLMS_TXT).toContain('LiveHive.start(attributes:contentState:)')
    expect(LLMS_TXT).toContain('Send test update')
    expect(LLMS_TXT).toContain('Paste https://github.com/iangdavis/livehive-ios')
  })
  ...
  it('getting started installs the published Swift package, not this repo', () => {
    ...
    expect(page).toContain('Apple Developer')
    expect(page).toContain('Widget Extension')
    expect(page).toContain('LiveHive.start')
    expect(page).toContain('Send test update')
    expect(page).toContain('attributesSnippet')
    expect(page).toContain('liveActivityWidgetSnippet')
    expect(page).not.toMatch(/<h2>Do not<\/h2>/)
    expect(snippets).toContain('print(activity.id)')
    expect(snippets).toContain('LiveHive.start(')
    expect(snippets).toContain('struct DeliveryAttributes')
    expect(snippets).toContain('struct DeliveryLiveActivity')
