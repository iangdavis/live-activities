...
async function reportAccountUsageForProject(projectId: string, units = 1) {
  const month = currentMonthKey()

  // update project-level UsageMonth (existing)
  await prisma.usageMonth.upsert({
    where: { projectId_month: { projectId, month } },
    create: { projectId, month, updates: units },
    update: { updates: { increment: units } },
  })

  // update account-level usage month
  const project = await prisma.project.findUnique({ where: { id: projectId }, select: { accountId: true } })
  if (!project) return
  const accountId = project.accountId
  await prisma.accountUsageMonth.upsert({
    where: { accountId_month: { accountId, month } },
    create: { accountId, month, units },
  ...
  if (account?.stripeSubscriptionItemId) {
    try {
      ...
      // log and continue; do not block user flow on billing failures
